...
data_11_2018 <- tbl_df(fread("data/2018_12_Desembre_qualitat_aire_BCN.csv", header=TRUE))
data_12_2018 <- tbl_df(fread("data/2018_11_novembre_qualitat_aire_BCN.csv", header=TRUE))
...
### Accidents
```{r}
accidents <- tbl_df(fread("2018_accidents_causes_gu_bcn.csv", header=TRUE))
accidents <- accidents %>% 
  select("Nom_districte", "Any", "Mes_any","Dia_mes", "Hora_dia", "Descripcio_causa_mediata") %>%
  mutate(sector = as.factor(Nom_districte),
         any = Any,
         mes = Mes_any,
         dia = Dia_mes, 
         hora = Hora_dia) %>%
  select(sector, any, mes, dia, hora)
levels(accidents$sector)
levels(data$sector)
```

```{r}
accidents %>%
  filter(sector %in% c("Eixample", "Gràcia"), any == 2018, mes >= 6) %>%
  group_by(sector, mes, hora) %>%
  summarise(n_acc = n()) %>%
  ggplot(aes(hora, n_acc)) +
  labs(title = "Número de accidentes",
       x = "Hora", 
       y = "Accidentes",
       caption = "Datos de opendata-ajuntament.barcelona.cat") +
  geom_line() +
  geom_point() + 
  facet_grid(sector ~ mes) +
  theme_minimal()
